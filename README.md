...
### 2. 環境構築後、画像が表示されない
* **原因:** Laravelの `storage/app/public` フォルダに保存されたユーザー投稿画像は、リポジトリの容量肥大化を防ぐため `.gitignore` によってGitの管理対象外（アップロードされない仕様）となっています。
* **解決策:** これはバグではなく正常な動作です。新しい環境で画像の表示テストを行いたい場合は、ブラウザ上のシステムから新しく商品をテスト出品して確認してください。
